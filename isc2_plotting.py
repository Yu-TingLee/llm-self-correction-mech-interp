...
def load(path: Path):
    ...
    round = np.array([r["round"] for r in d])
...
def main():
    ...
    series = [
        ("RoBERTa",  "detox", f"{model_basename}_RoBERTa_{args.strength}_detox_stats.json"),
        ("Detoxify", "detox", f"{model_basename}_Detoxify_{args.strength}_detox_stats.json"),
        ("RoBERTa",  "tox",   f"{model_basename}_RoBERTa_{args.strength}_tox_stats.json"),
        ("Detoxify", "tox",   f"{model_basename}_Detoxify_{args.strength}_tox_stats.json"),
    ]
    x_ticks = None
    colors = {
    ...
    for classifier, cond, fname in series:
        p = Path("outputs") / model_basename / f"{classifier}_{args.strength}_text_results" / fname
        x, mean, std = load(p)
        x_ticks = x
        ...
        ax.fill_between(
            x, mean - std, mean + std,
            color=c, alpha=band_alpha, linewidth=0, zorder=1
        )

        ax.plot(
            x, mean,
            color=c, linewidth=2.2, label=label, zorder=2
        )


    ax.set_title(f"{model_basename}", fontsize=16, weight="bold", pad=6)
    ax.set_xlabel("round", fontsize=14,labelpad=4)
    # ax.set_ylabel("toxicity", fontsize=11, labelpad=2)
    ...
    fig.savefig(
        os.path.join(output_dir, f"{model_basename}_{args.strength}_new.png")
    )
    plt.close(fig)
