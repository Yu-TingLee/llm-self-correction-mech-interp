...
def init_model(args, hook_hidden=True):
    tokenizer = AutoTokenizer.from_pretrained(args.model_dir, trust_remote_code=True,token=args.access_token)
    model = AutoModelForCausalLM.from_pretrained(args.model_dir, trust_remote_code=True,torch_dtype=torch.float16, 
                                                 ignore_mismatched_sizes=True,token=args.access_token)
...
def run_with_steering_vec(args, model, tokenizer, hidden_state_hooks):
    ...
    steering_vec_dir = f"outputs/{model_basename}/steering_d1_t100/1_vector/steer.json"
    ...
    for layer, v in list(steering_vec.items()):
        if v.ndim == 2:                 # (T, H)
            v = v.mean(axis=0)
            # steering_vec[layer] = v.mean(axis=0)  # (H,)
        elif v.ndim == 1:               # (H,)
            pass
    ...
    dialog_prefix = ["Think carefully and complete the rest of the sentence after the prompt.\nHere is the prompt to complete: "]
    ...
    for data_type, inj_coef in exp_specs:
        dataset = pd.read_json(f"./data/{data_type}_test_1k.jsonl", lines=True)
...
def generate_response(args, input_data, dialog_prefix, tokenizer, model, 
    ...
    for d in range(len(input_data)):
        ...
        chat_template = [
            {"role": "system", "content": "You are a helpful assistant and must follow my instructions. Always provide your completion as a single paragraph or sentence."},
        ...
        inputs = tokenizer(dialog_prompts_temp, truncation=True, max_length=max_input_length, 
                           padding='max_length', padding_side="left", return_tensors="pt").to(model.device)
        ...
        if not all("baseline_tox" in results[x] for x in dialog_ids_temp):
            ...
            baseline_tox = toxicity_evaluation(baseline_texts)
        ...
        for layer in layers_to_run:
            ...
            steered_tox = toxicity_evaluation(steered_texts)
...
def run():
    ...
    parser.add_argument("--inj_coef", type=float)
