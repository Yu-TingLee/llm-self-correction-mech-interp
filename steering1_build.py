...
def dataset_preprocessing(args):
    ...
    for data_tag in args.data_tags:
        ...
        data_nontoxic=f"data_processed/{data_tag}/nt_prompt_score.jsonl"
...
def init_model(args, hook_hidden=True):
    tokenizer = AutoTokenizer.from_pretrained(args.model_dir, trust_remote_code=True, local_files_only=False, token=args.access_token)
...
def get_embed_steering(args, model, tokenizer, hidden_states):
    ...
    for prompt_toxic, prompt_nontoxic in zip(t_prompts, nt_prompts):
        ...
        model(
            torch.tensor(ids_toxic, dtype=torch.long, device=model.device),
            use_cache=False
        )
        ...
        for layer, ascore in hidden_states.items():
            if layer in selected_layers:
                ...
                # embed_toxic[counts%sep][layer] += (ascore[-1].detach().cpu().numpy())
            ascore.clear()

        model(
            torch.tensor(ids_nontoxic, dtype=torch.long, device=model.device),
            use_cache=False
        )
        ...
        for layer, ascore in hidden_states.items():
            if layer in selected_layers:
                ...
                # embed_nontoxic[counts%sep][layer] += (ascore[-1].detach().cpu().numpy())
            ascore.clear()
